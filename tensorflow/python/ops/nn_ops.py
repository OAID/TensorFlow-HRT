...
def acl_lrn(input, depth_radius, bias, alpha, beta, name="AclLRN"):
  return gen_nn_ops._acl_lrn(input, depth_radius=depth_radius, bias=bias, alpha=alpha, beta=beta, name=None);
...
def acl_softplus(features, name=None):
  ...
  """
  with ops.name_scope(name, "AclSoftplus", [features]) as name:
    features = ops.convert_to_tensor(features, name="features")
    return gen_nn_ops._acl_softplus(features, name=name)
...
def acl_relu(features, name=None):
  """Computes Concatenated ReLU.
  ...
  Returns:
    A `Tensor` with the same type as `features`.
  """
  with ops.name_scope(name, "AclRelu", [features]) as name:
    features = ops.convert_to_tensor(features, name="features")
    return gen_nn_ops._acl_relu(features, name=name)
...
def acl_softmax(logits, dim=-1, name=None):
  """Computes softmax activations.
  ...
  Raises:
    InvalidArgumentError: if `logits` is empty or `dim` is beyond the last
      dimension of `logits`.
  """
  return _softmax(logits, gen_nn_ops._acl_softmax, dim, name)
...
def acl_avg_pool(value, ksize, strides, padding, data_format="NCHW", name=None):
  """Performs the average pooling on the input.
  ...
  Returns:
    A `Tensor` with the same type as `value`.  The average pooled output tensor.
  """
  with ops.name_scope(name, "AclAvgPool", [value]) as name:
    value = ops.convert_to_tensor(value, name="input")
    return gen_nn_ops._acl_avg_pool(value,
                                ksize=ksize,
                                strides=strides,
                                padding=padding,
                                data_format=data_format,
...
def acl_max_pool(value, ksize, strides, padding, data_format="NCHW", name=None):
  """Performs the max pooling on the input.
  ...
  Returns:
    ...
    The max pooled output tensor.
  """
  with ops.name_scope(name, "AclMaxPool", [value]) as name:
    value = ops.convert_to_tensor(value, name="input")
    return gen_nn_ops._acl_max_pool(value,
                                ksize=ksize,
                                strides=strides,
                                padding=padding,
                                data_format=data_format,
