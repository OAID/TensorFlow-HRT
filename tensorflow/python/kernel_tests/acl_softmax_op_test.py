...
class SoftmaxTest(test.TestCase):
  ...
  def _testSoftmax(self, np_features, dim=-1, log=False, use_gpu=False):
    ...
    with self.test_session(use_gpu=use_gpu):
      tf_softmax = nn_ops.acl_softmax(np_features, dim=dim, name=name)
      out = tf_softmax.eval()
  ...
  def _testAll(self, features):
    ...
    self._testSoftmax(features, log=False, use_gpu=True)
  ...
  def testNpSoftmax(self):
    ...
    self.assertAllClose(
        ...
        atol=1.e-5)
    """
    np_lsm = self._npSoftmax(np.array(features), log=False)
    self.assertAllClose(
  ...
  def testShapeInference(self):
    op = nn_ops.acl_softmax([[[1., 1., 1., 1.], [1., 2., 3., 4.]],
                         [[2., 3., 4., 5.], [6., 7., 8., 9.]],
                         [[5., 4., 3., 2.], [1., 2., 3., 4.]]])
  ...
  def testEmptyInput(self):
    with self.test_session():
      ...
      with self.assertRaises(errors_impl.InvalidArgumentError):
        nn_ops.acl_softmax(x, dim=0).eval()

  def testDimTooLarge(self):
    with self.test_session():
      with self.assertRaises(errors_impl.InvalidArgumentError):
        nn_ops.acl_softmax([1., 2., 3., 4.], dim=100).eval()
  """
